...
Dobra je praksa i odmah inicijalizirati \texttt{git} repozitorij u projektu,
što se može učiniti naredbom \texttt{git init}. To omogućuje praćenje promjena
i upravljanje verzijama k\^oda tijekom razvoja.

Također, preporučljivo je postaviti i formatiranje k\^oda, što se može postići
instalacijom i konfiguracijom alata poput Prettiera kako bi se osigurava
...
jest sljedeća:\\ \dirtree{.1 repeticije-hr/. .2 {.next/}. .2 app/. .2
  ...
  node\_modules/. .2 prisma/. .2 public/. .2 scripts/. .2 supabase/. .2
    {.editorconfig}. .2 {.env}. .2 {.env.example}. .2 {.gitignore}. .2
    {.prettierrc.json}. .2 components.json. .2 eslint.config.mjs. .2 next-env.d.ts.
  .2 next.config.ts. .2 package-lock.json. .2 package.json. .2
...
\begin{itemize}
  ...
  \item \texttt{.editorconfig} {--}  Datoteka s pravilima formatiranja k\^oda za različite editore.
  ...
  \item \texttt{postcss.config.mjs} {--}  Konfiguracija za PostCSS pipeline (npr. Tailwind CSS).
  ...
  \item \texttt{vercel.json} {--}  Konfiguracija \textit{deploya} na platformu Vercel, uključujući \textit{cron} zadatke i specifične postavke okruženja.
\end{itemize}

\subsubsection{Struktura \texttt{app/} direktorija}

Struktura \texttt{app/} direktorija zasebno je predstavljena zbog svoje ključne
uloge u organizaciji k\^oda i implementaciji poslovne logike aplikacije. Unutar
\texttt{app/} direktorija nalaze se sljedeći poddirektoriji i datoteke:\\
...
\begin{itemize}
  ...
  \item \texttt{layout.tsx} {--} Korijenski \texttt{layout} App Routera koji definira zajedničku strukturu stranica (npr.\ zaglavlje, podnožje i pružatelje konteksta).
...
Aplikacija koristi bazu podataka PostgreSQL \textit{hostanu} na platformi
Supabase kao centralno spremište svih trajnih podataka o korisnicima, ponudama
...
način kombiniraju prednosti Supabase platforme (upravljani PostgreSQL,
sigurnost, nadzor) i Prisma ORM-a (tipizirani upiti, migracije, generirani
klijent), bez potrebe za održavanjem vlastite baze.
...
Prisma ORM služi kao apstrakcijski sloj između domenskog koda i baze podataka
...
Migracije baze podataka kreirane su pomoću Prisma migracija, koje se izvršavaju
naredbom: \texttt{npx prisma migrate dev} nakon definiranja ili izmjene modela
u \texttt{schema.prisma}. Ova naredba generira SQL migracijske datoteke koje se
zatim primjenjuju na bazu podataka, stvarajući potrebne tablice i odnose.
...
\begin{itemize}
  ...
  \item \texttt{Session}, \texttt{Account}, \texttt{Verification} {--} tablice koje, uz \texttt{User}, koristi autentikacijska biblioteka \texttt{better\_auth}.
...
Next.js App Router koristi datotečnu strukturu unutar \texttt{app/} direktorija
...
aplikacija podržava, dok datoteke unutar \texttt{app/api/} direktorija
...
layout obavija pripadne stranice, čime se postiže nasljeđivanje i ponovno
korištenje zajedničkih elemenata kao što su navigacija, zaglavlje ili podnožje.
...
organiziraju na konzistentan način unutar istog \texttt{app/} direktorija, pri
čemu se čitljivost i održavanje koda poboljšavaju zahvaljujući jasnom
...
autentikacije (email/lozinka, OAuth i dr.), pri čemu sav osjetljivi podatkovni
sloj ostaje u PostgreSQL-u, a biblioteka generira tipizirani API za
...
slučaju email/lozinka). Na temelju te konfiguracije Better Auth generira
...
dodaci koje će koristiti korisničko sučelje. Ovaj modul izlaže funkcije za
...
registraciju i reset lozinke, čime se smanjuje dupliciranje koda i
pojednostavljuje upravljanje autentikacijom u korisničkom sučelju.

\begin{lstlisting}[caption={Konfiguracija Better biblioteke Auth i Admin plugina},label={lst:betterAuthConfig}]
...
kreiranja računa, izmjene uloga, zabrane (ban) i impersonacije
...
verzija ograničena je na skup CET/CEST zona koji pokriva ciljnu skupinu
korisnika.
...
Integracija Google Maps servisa u aplikaciji koristi Maps JavaScript API,
Geocoding i Places kako bi se omogućilo automatsko popunjavanje (engl.\
autocomplete) adresa, geokodiranje na poslužitelju te filtriranje i validacija
ponuda prema udaljenosti.
...
(latitude/longitude). Te koordinate služe i za prikaz lokacije na karti i za
lokalne UI filtre.
...
ponuda repeticija razdvajaju \textit{online} i onsite ponude, a kod slanja
obavijesti i e-mailova za \textit{online} lekcije adresa lokacije je
...
\subsection{Slanje obavijesti putem e-maila}

Sustav obavijesti putem e-maila temelji se na predlošcima i centralnom
...
sudionika, predmet, datum i vrijeme, način izvođenja itd.) i generira e-mail u
skladu s potrebama aplikacije.

U trenutnoj fazi razvoja koristi se zamjenska konfiguracija za lokalno i testno
slanje e-mailova na platformi Resend. Podešavanje uključuje definiranje
varijabli okruženja \texttt{RESEND\_API\_KEY} i
...
Primjer testnog dolaznog e-maila prikazan je na slici~\ref{fig:testEmail}.

\begin{figure}[H]
  \includegraphics[width=0.8\linewidth,clip]{assets/resend-test-email.png}
  \centering
  \caption{Primjer testnog e-maila poslanog preko Resend API-ja}\label{fig:testEmail}
\end{figure}
...
zapise čiji je izračunati kraj termina manji ili jednak trenutnom vremenu,
ažurira im status na \texttt{COMPLETED} i, po potrebi, šalje obavijesti
sudionicima.
...
Studenti, nakon prijave, dobivaju pristup nadzornoj ploči gdje imaju pristup
...
te ista ispravno radi kroz različite preglednike, operacijske sustave, uređaje
i mreže.
