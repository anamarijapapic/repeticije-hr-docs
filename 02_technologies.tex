...
okruženju Node.js. Paketi (biblioteke) se preuzimaju iz javnog repozitorija
...
PostgreSQL, uz objektno-relacijsko mapiranje pomoću Prisma ORM-a i upravljani
\textit{hosting} preko platforme Supabase.
...
\textbf{Prisma ORM} (engl.\ \textit{Object-Relational Mapper}) je biblioteka za
objektno-relacijsko mapiranje za Node.js i TypeScript koji omogućuje tipno
siguran (engl.\ \textit{type-safe}) pristup bazi podataka. U središtu Prisma
pristupa nalazi se datoteka \texttt{schema.prisma} u kojoj se deklarativno
definira shema podataka putem Prisma Schema Language (PSL) {--} definiraju se
modeli, relacije i mapiranja na tablice u bazi, a Prisma na temelju te sheme
automatski generira klijenta za pristup podacima, zajedno s odgovarajućim
TypeScript tipovima. Time se postiže da su sve upite prema bazi moguće statički
provjeriti u vrijeme prevođenja, uz bogatu podršku za automatsko dovršavanje i
otkrivanje neispravnih upita prije pokretanja aplikacije, što smanjuje broj
pogrešaka i ubrzava razvoj. Uz Prisma Client za rad s podacima, Prisma nudi i
alate za migracije (Prisma Migrate) te vizualni pregled podataka
(Prisma Studio) koji olakšavaju upravljanje razvojnim i produkcijskim
okruženjima.~\cite{prismaDocs}.
...
tipno siguran sloj pristupa podacima u okruženju Next.js/TypeScript, dok
...
komunikacije u stvarnom vremenu između tutora i učenika tijekom online
...
\begin{figure}[H]
    ...
    \centering
    \caption{WebRTC kao kolekcija protokola (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/01-webrtc-agent.png}{webrtcforthecurious.com}})}\label{fig:webrtcAgent}
\end{figure}
...
aplikacija, od video konferencija, do online igara, udaljenog upravljanja, IoT
(engl.\ \textit{Internet of Things}) komunikacije i drugih scenarija koji
zahtijevaju nisku latenciju i visoku kvalitetu prijenosa.
...
implementaciju i bez dodatne poslužiteljske infrastrukture za medije.
Alternativno, \textit{hybrid mesh} smanjuje broj izravnih veza tako da se
mediji prosljeđuju preko pojedinih sudionika.
...
\begin{figure}[H]
    ...
    \centering
    \caption{Topologija „1 na 1” (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/08-one-to-one.png}{webrtcforthecurious.com}})}\label{fig:webrtcOneToOne}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=1\linewidth,clip=]{assets/webrtc-full-mesh.png}
    \centering
    \caption{Topologija \textit{full mesh} (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/08-full-mesh.png}{webrtcforthecurious.com}})}\label{fig:webrtcFullMesh}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=1\linewidth,clip=]{assets/webrtc-hybrid-mesh.png}
    \centering
    \caption{Topologija \textit{hybrid mesh} (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/08-hybrid-mesh.png}{webrtcforthecurious.com}})}\label{fig:webrtcHybridMesh}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=0.8\linewidth,clip=]{assets/webrtc-sfu.png}
    \centering
    \caption{Topologija SFU (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/08-sfu.png}{webrtcforthecurious.com}})}\label{fig:webrtcSFU}
\end{figure}

\begin{figure}[H]
    \includegraphics[width=0.8\linewidth,clip=]{assets/webrtc-mcu.png}
    \centering
    \caption{Topologija MCU (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/08-mcu.png}{webrtcforthecurious.com}})}\label{fig:webrtcMCU}
\end{figure}
...
\begin{figure}[H]
    ...
    \centering
    \caption{Signalizacija (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/01-signaling-sequence-overview.png}{webrtcforthecurious.com}})}\label{fig:webrtcSignaling}
\end{figure}
...
uvijek preferiraju najizravnije i najmanje „skupe”
putanje~\cite{webrtcForTheCurious}.

\begin{figure}[H]
    \includegraphics[width=0.5\linewidth,clip=]{assets/webrtc-connectivity-checks.png}
    \centering
    \caption{ICE kandidati i testiranje parova (Izvor: \texttt{\href{https://webrtcforthecurious.com/docs/images/03-connectivity-checks.png}{webrtcforthecurious.com}})}\label{fig:webrtcConnectivityChecks}
\end{figure}
...
TLS-a koji se koristi u HTTPS-u, zadužen za sigurno pregovaranje kriptografskih
parametara i razmjenu ključeva preko nepouzdanog UDP transporta, pri čemu štiti
od prisluškivanja, krivotvorenja poruka i njihove izmjene u
...
autentificiran, bez mogućnosti isključenja enkripcije u produkcijskim
...
promet prenosi kao SRTP paketi preko ranije uspostavljenog sigurnog kanala. Za
proizvoljne podatkovne tokove WebRTC koristi SCTP (engl.\ \textit{Stream
...
ekstenzije za već prethosno spomenute tehnologije. \textbf{\texttt{git}} je
korišten za verzioniranje k\^oda, uz \textbf{GitHub} kao odabranu platformu za
objavu repozitorija. \textbf{Vercel} je odabran kao platforma za
\textit{hosting} i \textit{deployment} Next.js aplikacije, zbog svoje duboke
integracije s Next.js-om, automatskog skaliranja, globalne mreže isporuke
sadržaja (CDN) i jednostavnog procesa isporuke putem integracije s GitHub-om.
